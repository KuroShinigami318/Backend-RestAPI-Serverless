...
def GetAuth(filePath):
    ...
    if (not data):
        scriptLocation = os.getcwd() + '/GetAuth.bat'
        r = subprocess.call([scriptLocation])
        if (r != 0):
           print('Error while execute GetAuth batch script')
           return False
    ...
    if ('expiresAt' not in data or datetime.datetime.fromisoformat(data['expiresAt']) < now):  
        ...
        r = requests.post(url='https://securetoken.googleapis.com/v1/token?key=AIzaSyBDPRtq6-RLD713Im-BXwj4Fnz0O0A_Pkw', headers=header, data=body)
...
if (GetAuth(curDir + '/auth.json')):
    ...
    Login(backend_url + "/login", data['idToken'], "3118412003", "")